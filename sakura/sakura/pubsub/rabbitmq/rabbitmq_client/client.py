...
class RabbitMQClient:
    ...
    def __init__(self, settings: Settings):
        ...
        self.virtualhost = settings.virtualhost
        self.encoding = settings.encoding
        self.content_type = settings.content_type

        self.__queues_to_declare = settings.declare.queues
        self.__exchanges_to_declare = settings.declare.exchanges
    ...
    async def setup(self):
        ...
        for exchange in self.__exchanges_to_declare:
            if exchange.name:
                await self.get_exchange(exchange)

        for queue in self.__queues_to_declare:
            await self.get_queue(queue, declare=True)

    @staticmethod
    async def __get_message(queue: AbstractQueue, timeout: int = 1) -> Optional[AbstractIncomingMessage]:
